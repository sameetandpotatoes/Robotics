...
int speedArmAuton = 60;
int speedArmUserControl = 70;
	...
	if (SensorValue[disableAut] == 1){ //if auton sensor is not pressed
		...
		*/
		...
		while(scoredSections < 6){
			...
			// TODO: Change regression equation
			int calculatedHeight =
				(firstTime) ? 235 : (289.0 * scoredSections) + 85.33;
...
task usercontrol(){
	...
	while (true){
		...
		//Pneumatics Control
		SensorValue[pneuVal] =
			(vexRT[Btn7U] || vexRT[Btn7UXmtr2]) ? 1 :
		...
		//Button to return lift to the ground
		loweringLift = (vexRT[Btn8D] || vexRT[Btn8DXmtr2]) ? true : loweringLift;
		if (loweringLift && abs(SensorValue[shaftEncoder] > 10) && !killSwitch){
			liftMotors(-1);
			killSwitch = (vexRT[Btn8U] || vexRT[Btn8UXtmr2]) ? true : killSwitch;
		} else{
			stopLift();
			loweringLift = false;
			killSwitch = false;
		...
		//Button to pick up next skyrise
		if (vexRT[Btn8R] || vexRT[Btn8RXmtr2]){
			nextSkyrise();
			//Reset killswitch in case it was activated last time
			killSwitch = false;
		}
	}
}
